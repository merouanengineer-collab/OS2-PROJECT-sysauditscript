...
#   ./audit.sh --auto             # Run full audit, generate all reports, and email PDF
...
# SCRIPT_DIR is defined before this, so it's available for python_script path
for arg in "$@"; do
...
_run_full_audit() {
    local mode="${1:-full}"
    export AUDIT_CURRENT_STEP=0
    export AUDIT_TOTAL_STEPS=18
    log_info "Starting ${mode} audit on $(hostname)..."

    collect_all_hardware
    collect_all_software
    ((AUDIT_CURRENT_STEP+=2)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"
    echo -e "\n"
...
_show_help() {
    cat <<HELP
${BOLD}SysAudit v1.1 (PDF Enabled)${RESET} — Linux Audit & Monitoring System

${BOLD}Usage:${RESET}
  ./audit.sh [option]


${BOLD}Options:${RESET} (Some features require root privileges)
  ...
  --help          Show this help
  --auto          Run full audit, generate all reports (including PDF), and email PDF
  --test-ssh      Test SSH connection to remote host
  --debug         Enable verbose debug logging

${BOLD}Examples:${RESET}
  # Interactive menu (recommended for guided operations)
  sudo ./audit.sh
...
${BOLD}Prerequisites for PDF generation:${RESET}
  Ensure Python3 and WeasyPrint are installed:
...
_interactive_menu() {
    ...
    select choice in "${options[@]}"; do
        case "$REPLY" in
            ...
            14) rotate_logs ;;
            15) verify_pdf_toolchain ;;
            16) show_report_stats ;;
            17) _show_help ;;
            18|q|exit) echo -e "${GREEN}Goodbye!${RESET}"; exit 0 ;;
            *)  echo -e "${RED}Invalid choice. Enter 1-18.${RESET}" ;;
        esac
...
case "${1:-}" in
    --auto)
        print_banner
        _run_full_audit "full"
        send_report_email "${EMAIL_RECIPIENT}" "full"
        ;;

...
print_software_full() {
    log_section "[ DETAILED SOFTWARE AUDIT ]"
    echo -e "${BOLD}Operating System:${RESET} ${SW_DATA[os_name]:-N/A}"
    echo -e "${BOLD}Kernel Version:${RESET}   ${SW_DATA[kernel_version]:-N/A}"
    echo -e "${BOLD}System Uptime:${RESET}    ${SW_DATA[uptime]:-N/A}"
    echo -e "\n${BOLD}Top 5 Processes (CPU):${RESET}"
    echo "${SW_DATA[proc_top_cpu]:-N/A}" | head -n 6
    echo -e "\n${BOLD}Running Services (sample):${RESET}"
    echo "${SW_DATA[services_running]:-N/A}" | head -n 10
    echo -e "\n${BOLD}Active Listening Ports:${RESET}"
    echo "${SW_DATA[ports_listening]:-N/A}" | head -n 10
    [[ -n "${SW_DATA[services_failed]}" ]] && \
        echo -e "\n${RED}${BOLD}Failed Systemd Services:${RESET}\n${SW_DATA[services_failed]}"
}

# ── Main collector ────────────────────────────────────────────────────────────────
collect_software() {
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_os_info
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_packages
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_users
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_services
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_processes
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_ports
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_security
    ((AUDIT_CURRENT_STEP++)); show_progress "$AUDIT_CURRENT_STEP" "$AUDIT_TOTAL_STEPS"; collect_environment
}
